    ...
    *args
    ...
    for env_name in env_names:
        ...
        if value_network is None:
            value_network = create_simple_conv(latent_obs_dim, act_dim, actv='relu', output_actv='sigmoid', seed_i=555)
        
        ALG = ProximalPG(ENV, policy, value_network, *args)
        ALG.entropy_rate = 1.
        ALG._value_reg = 0.1
...
def run_atari_vg(env_names : list, epoches=40, policy=None, value_network=None, latent_obs_dim=(100, 100, 1), dir_='Run', 
    *args
    ...
    for env_name in env_names:
        ...
        ALG = VanillaPG(ENV, policy, value_network, *args)
        ALG.entropy_rate = 1.
        ALG._value_reg = 0.1
        ALG.value_loss_const = 0.7
        ALG.pol_iters = 70 
...
def run_atari_rf(env_names : list, epoches=40, policy=None, value_network=None, latent_obs_dim=(100, 100, 1), dir_='Run', 
    *args
    ...
    for env_name in env_names:
        ...
        ALG = ReinforcePG(ENV, policy, *args)
        ALG.entropy_rate = 1.
        ALG._value_reg = 0.1
        ALG.model_save_dir = 'Models/{}/{}'.format(env_name, type(ALG).__name__)
...
N0 = run_atari_ppo(env_names : list, epoches=30, policy=None, value_network=None, latent_obs_dim=(100, 100, 1), dir_='Run', 
        gamma=0.99, max_trj = 3000, clip_ratio=0.3, kl_beta=0, kl_target=0.001, kl_target=0.001, kl_tol=2, kl_low=1.5, kl_cutoff=0.0,
        grad_clip_norm=2., policy_lr=0.001, value_lr=0.01
    )

N1 = run_atari_vg(env_names : list, epoches=30, policy=None, value_network=None, latent_obs_dim=(100, 100, 1), dir_='Run', 
        gamma=0.98, max_trj=3000, summary=True, optimizer=None, grad_clip_norm=2., policy_lr=0.001, value_lr=0.01, use_gae=True, lam=0.95
    )

N2 = run_atari_rf(env_names : list, epoches=30, policy=None, value_network=None, latent_obs_dim=(100, 100, 1), dir_='Run',
        gamma=0.98, max_steps=10000, optimizer=None, grad_clip_norm=2., policy_lr=0.001
    )
